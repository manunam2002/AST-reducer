...
CREATE TABLE t1 (c0 INTEGER PRIMARY KEY, c1 INTEGER, c2 DATE, c3 TEXT, c4 MEDIUMINT, c5 INTEGER, c6 REAL);
...
CREATE TABLE t2 (c0 REAL PRIMARY KEY, c1 NONE, c2 REAL);
...
CREATE TABLE t3 (c0 REAL PRIMARY KEY, c1 DATE, c2 BLOB, c3 INTEGER, c4 TEXT, c5 TEXT, c6 TEXT, c7 TEXT);
...
CREATE TABLE t4 (c0 TEXT PRIMARY KEY, c1 TEXT, c2 INTEGER);
...
CREATE TABLE t5 (c0 TEXT PRIMARY KEY, c1 MEDIUMINT, c2 TEXT);
...
WITH t1_stats AS (SELECT c1, COUNT(*) AS count, AVG(c0) AS avg_pk FROM t5 GROUP BY c1), t2_derived AS (SELECT c0, c3, CASE WHEN c3 IS NULL THEN 'Unknown' WHEN c3 < 50 THEN 'Low' ELSE 'High' END AS category FROM t3) SELECT main.*, (SELECT AVG(count) FROM t1_stats) AS overall_avg, (SELECT COUNT(*) FROM (SELECT t3.c0, LAG(t3.c4) OVER (ORDER BY t3.c0) AS prev_val, LEAD(t3.c4) OVER (ORDER BY t3.c0) AS next_val FROM t0 AS t3 WHERE t3.c0 IN (SELECT td.c0 FROM t2_derived AS td WHERE td.category = main.category UNION SELECT ts.avg_pk FROM t1_stats AS ts WHERE ts.c1 = main.c1)) AS complex_window WHERE NOT complex_window.prev_val IS NULL OR NOT complex_window.next_val IS NULL) AS window_matches FROM (SELECT ts.c1, td.category, ts.count, ts.avg_pk, DENSE_RANK() OVER (PARTITION BY td.category ORDER BY ts.count DESC) AS rank_in_category FROM t1_stats AS ts CROSS JOIN (SELECT DISTINCT category FROM t2_derived) AS td) AS main WHERE main.rank_in_category <= 2 ORDER BY main.category, main.rank_in_category