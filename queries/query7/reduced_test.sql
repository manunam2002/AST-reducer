...
SELECT qryrdu.c1 AS pcaksy FROM (SELECT * FROM main.view0 AS ysjmlj CROSS JOIN view0 WHERE FALSE) AS qryrdu WHERE FALSE;
SELECT svslfg.c1 AS hghlqv, view0.c0 AS ujmumo FROM (view0) AS svslfg CROSS JOIN view0 WHERE TRUE;
REPLACE 'INTO t1 VALUES (-73, 2088, 1989)';
CREATE VIEW view1 AS VALUES ((x'7067e3cec226b60e' % 904.1747253662293));
SELECT ufulnp.c0 AS trwhwf, ufulnp.c1 AS lcaqvk FROM (t0) AS ufulnp WHERE FALSE;
INSERT INTO t0 VALUES (6110291749224401160, -3312);
PRAGMA integrity_check;
SELECT view0.c0 AS rzhvww FROM view0 CROSS JOIN (view0) AS olntlj CROSS JOIN view1 WHERE FALSE;
SELECT * FROM (view1) AS gwydaz WHERE ABS(-6002);
CREATE VIEW view2 (c0) AS SELECT fbuhaj.c1 AS eoswuc FROM view1 CROSS JOIN (t0) AS fbuhaj CROSS JOIN view1 AS xlksxh CROSS JOIN (VALUES (-9278)) AS febily WHERE FALSE;
CREATE VIEW view3 (c0, c1, c2, c3, c4, c5, c6, c7, c8, c9) AS SELECT * FROM t0 AS stjdgv CROSS JOIN (view1) AS czdvgs CROSS JOIN (SELECT apfspc.c0, apfspc.c2 FROM (main.t1 AS apfspc JOIN view1 ON 1) WHERE TRUE) AS gvihsa CROSS JOIN (SELECT t2.c1, t2.c0, jnebub.c0 AS eadjzj, jnebub.c1, jnebub.c2, t2.c2 FROM view1 CROSS JOIN t2 CROSS JOIN t2 AS jnebub WHERE UPPER(t2.c0)) AS kljnyt WHERE FALSE;
